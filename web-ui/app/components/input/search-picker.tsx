...
export function SearchPickerButton({ disabled = false, className }: SearchPickerButtonProps) {
  ...
  const selectedServiceIndices = React.useMemo(() => {
    if (!settings) {
      return [];
    }

    const rawIndices = Array.isArray(settings.searchServiceSelectedIndices)
      ? settings.searchServiceSelectedIndices
      : [settings.searchServiceSelected];

    return rawIndices
      .filter(
        (index): index is number =>
          Number.isInteger(index) && index >= 0 && index < settings.searchServices.length,
      )
      .filter((index, position, list) => list.indexOf(index) === position)
      .sort((left, right) => left - right);
  }, [settings]);
  const currentService =
    selectedServiceIndices.length === 1
      ? settings?.searchServices?.[selectedServiceIndices[0]] ?? null
      : null;
  ...
  const updateSearchProvidersMutation = useMutation({
    mutationFn: ({ indices }: { indices: number[] }) =>
      api.post<{ status: string }>("settings/search/providers", { indices }),
  ...
  const loading =
    ...
    updateSearchProvidersMutation.isPending;
                  ...
                  {settings?.searchServices?.length ? (
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                      {settings.searchServices.map((service, index) => {
                        const selected = selectedServiceIndices.includes(index);
                        const switching =
                          updateSearchProvidersMutation.isPending &&
                          updateSearchProvidersMutation.variables?.indices.includes(index);
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              const nextIndices = selected
                                ? selectedServiceIndices.filter((item) => item !== index)
                                : [...selectedServiceIndices, index].sort((left, right) => left - right);
                              updateSearchProvidersMutation.mutate({ indices: nextIndices });
                            }}
                          >
                            <AIIcon
